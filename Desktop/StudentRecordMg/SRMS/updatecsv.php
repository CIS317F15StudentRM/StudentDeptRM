			...
			for($j=0; $j<$numkey; $j++)
			{
				...
				if(!empty($result[$i]['ID']) && $keys[$j] == "ID"){
					...
					if(!empty($result[$i]['Course Number']) && $keys[$j] == "Course Number"){
						...
						$returncall =coursetype($result[$i]['ID'],$coursenum);
...
for($i=0; $i<=$numofstudent ; $i++){
   ...
   if($add == 1){
		...
		echo "window.location.href ='updatecsv.php';";
		echo '</script>';
    ...
    <div id="wrapper">
      ...
      <div class="main">
        <div class="formwrapper">
          <form class="form" method="post" action="updatecsv.php" enctype="multipart/form-data">
          <table id="semyear">
           <tr class="name">
